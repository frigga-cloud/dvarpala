...
func (az *AzureProvider) InstallDvarpalaDirectly(instanceInfo *AzureInstanceInfo, config InstanceConfig, keyPath string) error {
	...
	steps := []struct {
		...
		{"Setting up Easy-RSA", "sudo make-cadir /opt/dvarpala/easy-rsa && sudo chown -R azureuser:azureuser /opt/dvarpala"},
	...
	if err := az.executeSSHCommand(instanceInfo.PublicIP, "sudo cp /opt/dvarpala/certs/admin.ovpn /var/www/html/admin.ovpn && sudo chmod 644 /var/www/html/admin.ovpn", keyPath); err != nil {
...
sudo chown azureuser:azureuser /opt/dvarpala/certs/admin.ovpn
sudo chown azureuser:azureuser /opt/dvarpala/certs/admin-credentials.txt
`
