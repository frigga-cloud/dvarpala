...
func (gcp *GCPProvider) InstallDvarpalaDirectly(instanceInfo *GCPInstanceInfo, config InstanceConfig, keyPath string) error {
	...
	steps := []struct {
		...
		{"Setting up Easy-RSA", "make-cadir /home/$(whoami)/dvarpala/easy-rsa && ls -la /home/$(whoami)/dvarpala/"},
		{"Configuring Easy-RSA vars", gcp.getEasyRSAVarsCommand(config)},
		{"Building Certificate Authority", "cd /home/$(whoami)/dvarpala/easy-rsa && ./easyrsa init-pki  && ./easyrsa --batch build-ca nopass && ls -la pki/"},
		{"Generating server certificate", "cd /home/$(whoami)/dvarpala/easy-rsa && ./easyrsa --batch build-server-full server nopass && ls -la pki/issued/ && ls -la pki/private/"},
		{"Generating admin client certificate", "cd /home/$(whoami)/dvarpala/easy-rsa && ./easyrsa --batch build-client-full admin nopass && ls -la pki/issued/ && ls -la pki/private/"},
		{"Generating TLS authentication key", "cd /home/$(whoami)/dvarpala/easy-rsa && openvpn --genkey --secret pki/ta.key && ls -la pki/ta.key"},
		{"Copying certificates to OpenVPN directory", "sudo cp /home/$(whoami)/dvarpala/easy-rsa/pki/ca.crt /home/$(whoami)/dvarpala/easy-rsa/pki/issued/server.crt /home/$(whoami)/dvarpala/easy-rsa/pki/private/server.key /home/$(whoami)/dvarpala/easy-rsa/pki/ta.key /etc/openvpn/server/ && sudo ls -la /etc/openvpn/server/"},
		{"Creating OpenVPN server configuration", gcp.getOpenVPNServerConfigCommand()},
		{"Starting OpenVPN server", "sudo systemctl enable openvpn-server@server && sudo systemctl start openvpn-server@server && sudo systemctl status openvpn-server@server --no-pager"},
	...
	if err := gcp.executeSSHCommand(instanceInfo.ExternalIP, "sudo cp /home/$(whoami)/dvarpala/certs/admin.ovpn /var/www/html/admin.ovpn && sudo chmod 644 /var/www/html/admin.ovpn && ls -la /var/www/html/admin.ovpn && wc -l /var/www/html/admin.ovpn && curl -s -I http://localhost/admin.ovpn", keyPath); err != nil {
...
func (gcp *GCPProvider) getEasyRSAVarsCommand(config InstanceConfig) string {
