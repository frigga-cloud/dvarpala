...
func vpnIssueCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println()
			fmt.Println()
			fmt.Println("<SSH-KEY> is the .pem file that gets you on to this machine -")
			fmt.Println("not the .ovpn above. Those are different files with different")
			fmt.Println("jobs: the .pem is yours and opens this server, the .ovpn is")
			fmt.Println("theirs and opens the VPN. Give the full path to it, or the name")
			fmt.Println("from your ~/.ssh/config if you have one, in which case it is")
			fmt.Println("simply:")
			fmt.Println()
			fmt.Printf("  ssh <host> 'dvarpala-cli vpn issue --user %s --output -' > %s.ovpn\n",
				email, filepath.Base(email))
			fmt.Println()
			fmt.Println("Hand the file over directly - not by email - then remove the copy")
			fmt.Println("left here:")
